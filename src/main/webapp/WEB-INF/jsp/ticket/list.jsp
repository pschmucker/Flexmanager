...
<table border="2" cellspacing="2" cellpadding="5">
	...
	<c:forEach items="${list}" var="ticket">
		<tr>
			<td><a href="<c:url value="/ticket/view.html?id=${ticket.id}" />"><c:out value="${ticket.id}"/></a></td>
			<td><a href="<c:url value="/ticket/view.html?id=${ticket.id}" />"><c:out value="${ticket.title}"/></a></td>
			<td><fmt:formatDate pattern="dd/MM/yyyy HH:mm:ss" value="${ticket.creationDate}"/></td>
			<td><c:out value="${ticket.status}"/></td>
			<td><c:out value="${ticket.priority}"/></td>
			<td><a href="<c:url value="/client/view.html?id=${ticket.client.id}" />"><c:out value="${ticket.client.name}"/></a></td>
			<td><a href="<c:url value="/product/view.html?id=${ticket.product.id}" />"><c:out value="${ticket.product.name}"/></a></td>
